# Post-mail asset descriptions — Deutsch (DE)

> Nur den Text unter `DESC:` für jedes Asset bearbeiten.
> Die Zeilen `ID:`, `LABEL:` und `LINK:` unverändert lassen, damit die
> Änderungen zurück nach `web/src/lib/change-options.ts` übernommen werden können.
>
> Jedes Asset erscheint als eine Link-Karte in der Trainings-Recap-E-Mail:
>
> ```
> ➡️ [LABEL](URL)
> DESC
> ```

---

## 1. Trainings-Folien (Kern — standardmäßig enthalten)

Erscheinen unter `📂 Trainings-Folien` in der E-Mail.

### ID: material_intro
LABEL: Einführungstraining für die Elios 3
LINK: INTRO_TRAINING_URL
DESC:
Foliensatz von Tag 1 mit jeder Funktion, jedem Bedienelement und jeder Menüeinstellung der Elios 3, Schritt für Schritt erklärt—praktisch als Nachschlagewerk.

### ID: material_aiim
LABEL: Indoor Aerial Inspection Methodology (AIIM) Training
LINK: AIIM_TRAINING_URL
DESC:
Foliensatz von Tag 2–3 zu Missionsvorbereitung, Erkundung, Inspektionsdurchführung und Abschluss, Schritt für Schritt—hilfreich für komplexe Einsätze.

### ID: material_method_statement
LABEL: Method Statement Vorlage
LINK: METHOD_STATEMENT_URL
DESC:
Vorlage mit allen Missionsfragen, die ihr vor dem Flug mit dem Kunden klären solltet—Umfang, Zugang, Sicherheit, Ergebnisse—so nutzen oder anpassen.

### ID: material_risk_assessment
LABEL: Leitfaden zur Risikobewertung
LINK: RISK_ASSESSMENT_URL
DESC:
Bewertungsbogen, der Missionen nach menschlichen, drohnen- und umweltbezogenen Risikofaktoren in gewichteten Kategorien einstuft—hilft beim Einschätzen des Einsatzes.

### ID: material_sop
LABEL: SOP (Standard Operating Procedure)
LINK: SOP_URL
DESC:
Handbuch mit allen Schritten, Checks und Workflow-Referenzen vor und nach der Inspektion in einem Dokument—als Referenz oder ausgedruckte Checkliste nutzbar.

---

## 2. Weitere nützliche Links

Erscheinen unter `🔗 Weitere nützliche Links` (keine Unterüberschrift in der E-Mail).

### ID: useful_knowledge_base
LABEL: Flyability Knowledge Base
LINK: KNOWLEDGE_BASE_URL
DESC:
Flyability-Hub mit FAQs, Firmware-Hinweisen, Troubleshooting-Artikeln, Elios-3-Produktguides und Payload-Dokumentation—als zentrales Nachschlagewerk nutzen.

### ID: useful_youtube
LABEL: YouTube-Kanal
LINK: YOUTUBE_CHANNEL_URL
DESC:
Flyability-YouTube-Kanal mit Tutorial-Playlists, Payload-Walkthroughs und Branchen-Fallstudien—ideal für schnelle Auffrischer.

### ID: useful_customer_toolkit
LABEL: Customer Toolkit
LINK: CUSTOMER_TOOLKIT_URL
DESC:
Herunterladbare Elios-3-Markenassets, Broschüren, Datenblätter, Beispieldatensätze und Marketing-Medien—für eigene Kundenunterlagen wiederverwendbar.

### ID: useful_inspector
LABEL: Inspector 5
LINK: INSPECTOR_URL
DESC:
Flyabilitys Elios-3-Flugdatensoftware zum Herunterladen, Ansehen und Auswerten der Aufnahmen an einem Ort (stets die aktuelle Version unter diesem Link).

### ID: useful_nubigon
LABEL: Nubigon
LINK: NUBIGON_URL
DESC:
Drittanbieter-Software für Punktwolken-Animationen und kinoreife Flüge durch Elios-3-Scans (Abonnement nötig)—nützlich für Präsentationen.

### ID: useful_scan_bim
LABEL: Scan-to-BIM-Workflow
LINK: SCAN_TO_BIM_WORKFLOW_URL
DESC:
Kurzes Tutorial mit Schritten, Software und Exporteinstellungen, um eine Elios-3-Punktwolke in ein BIM-Modell umzuwandeln.

---

## 3. Weitere Trainings-Folien

Erscheinen unter `### 📑 Weitere Trainings-Folien`.

### ID: useful_intro_ut
LABEL: UT-Einführungstraining Elios 3 — Foliensatz
LINK: INTRO_UT_TRAINING_URL
DESC:
Leichter Einführungsfoliensatz zu Hardware, erster Inbetriebnahme und Ultraschall-Grundlagen des UT-Payloads—vor den ersten UT-Einsätzen nutzen.

### ID: useful_ut_advanced
LABEL: UT-Fortgeschrittenentraining Elios 3 — Foliensatz
LINK: UT_ADVANCED_ELIO3_DECK_URL
DESC:
Vollständiger UT-Payload-Foliensatz zu Sondentechnologie, Dual-Crystal-Mechanik, Messtheorie und Inspektionsmethodik—für jeden UT-Einsatz nutzbar.

### ID: useful_ut_probe
LABEL: Flyability-Leitfaden zur UT-Sondenauswahl — Foliensatz
LINK: UT_PROBE_SELECTION_GUIDE_URL
DESC:
Entscheidungsleitfaden mit Vergleich von Dual- und Single-Crystal-UT-Sonden und Auswahlkriterien nach Oberfläche, Beschichtung und Umgebung—vor der Missionsvorbereitung nutzen.

### ID: useful_faro_deck
LABEL: FARO Connect Training — Foliensatz
LINK: FARO_CONNECT_DECK_URL
DESC:
Surveyor-Foliensatz für REV 7 zum gesamten Workflow von der Aufnahme bis zur sauberen Punktwolke in FARO Connect, inkl. Flug-Best-Practices—als Referenz beim Verarbeiten.

### ID: useful_water_wastewater_deck
LABEL: Water & Wastewater Training — Foliensatz
LINK: WATER_WASTEWATER_DECK_URL
DESC:
Foliensatz zu Risiken, Sicherheitsaspekten, Flug-Best-Practices und Target-basierter Georeferenzierung in Kanälen und unterirdischen Leitungen—als Referenz vor ähnlichen Einsätzen.

### ID: useful_cement_deck
LABEL: Zementwerk- & Ofeninspektion — Foliensatz
LINK: CEMENT_ONLINE_DECK_URL
DESC:
Foliensatz zu Zementanlagen, Risikopunkten, Flug-Best-Practices und Inspektionsbeispielen an Ofen, Kühler und typischen Anlagen—als Referenz vor ähnlichen Einsätzen.

---

## 4. Online-Schulungen

Erscheinen unter `### 💻 Online-Schulungen`.

### ID: useful_faro_online
LABEL: FARO Connect Online-Kurs
LINK: FARO_CONNECT_ONLINE_COURSE_URL
DESC:
Selbstgeführter Academy-Kurs mit Modulen und Quizfragen zur Punktwolkenverarbeitung und zum FARO-Connect-Workflow von A bis Z—im eigenen Tempo durcharbeiten.

### ID: thinkific_regulation
LABEL: Regulatorik Kurs
LINK: (Thinkific — Flyability Academy Regulation)
DESC:
Academy-Kurs mit Modulen zu Luftfahrtregeln, Luftraumklassen, Behördengenehmigungen und operativer Compliance—vor regulierten Flügen nützlich.

### ID: thinkific_gas_sensor
LABEL: Gas Sensor Kurs
LINK: (Thinkific — Flyability Academy Gas sensor)
DESC:
Academy-Kurs zum Gas-Sensor-Payload—Montage, Einsatz im Flug, Alarmschwellen und Ablesen brennbarer Gastrends—vor den ersten Gas-Einsätzen nutzen.

### ID: thinkific_cement
LABEL: Zement-Industrie Kurs
LINK: (Thinkific — Flyability Academy Cement)
DESC:
Academy-Kurs zu Zementwerk-Inspektionen mit der Elios 3—Öfen, Kühler, Silos und typische heiße, staubige Anlagen—vor dem Einsatz vor Ort nützlich.

### ID: thinkific_mining
LABEL: Bergbau Kurs
LINK: (Thinkific — Flyability Academy Mining)
DESC:
Academy-Kurs zu Untertage-Inspektionen im Bergbau—Strecken, Abbaukammern, Überhauen, Erzrutschen und Gebirgssicherung—vor dem Einsatz vor Ort nützlich.

### ID: thinkific_wastewater
LABEL: Abwasser Kurs
LINK: (Thinkific — Flyability Academy Wastewater)
DESC:
Academy-Kurs zu Abwasser-Workflows—Kanalnetze, Kläranlagen, Zugangspunkte und Inspektionsberichte—vor ähnlichen Einsätzen nützlich.

### ID: thinkific_faro_connect
LABEL: FARO Connect Online-Kurs
LINK: (Thinkific — Flyability Academy FARO Connect)
DESC:
Geführter Academy-Kurs mit praktischen FARO-Connect-Übungen an Elios-3-Exports, vom Import bis zu kundenfähigen Ergebnissen—im eigenen Tempo durcharbeiten.

### ID: useful_wastewater_course
LABEL: Wastewater Online-Kurs
LINK: WASTEWATER_ONLINE_COURSE_URL
DESC:
Academy-Kurs zu Abwasser-Workflows—Kanalnetze, Kläranlagen, Zugangspunkte und Inspektionsberichte—vor ähnlichen Einsätzen nützlich.

### ID: useful_academy_hub
LABEL: Flyability Academy
LINK: FLYABILITY_ACADEMY_HUB_URL
DESC:
Flyability-Academy-Hub mit allen kostenlosen Branchen- und Payload-Kursen, Anmeldung und Fortschrittsübersicht—nach dem Training zum Vertiefen nutzen.

---

## 5. Schulungsvideos

Erscheinen unter `### 🎬 Schulungsvideos` (Abschnitts-Intro unten).

### SECTION_INTRO (videos)
Kurze Videos zu Payloads, Stromversorgung, Tether und Akkus—für Piloten und Unterstützung vor Ort, bevor solche Einsätze anstehen.

### ID: useful_gas_sensor
LABEL: Gas Sensor Quick-Start-Guide
LINK: GAS_SENSOR_QUICKSTART_URL
DESC:
Knowledge-Base-Quick-Start zum Gas-Sensor—Montage, Preflight, Alarmschwellen und Ablesen im Flug—vor den ersten Gas-Einsätzen ansehen.

### ID: useful_rad_video
LABEL: Elios 3 RAD-Sensor Schulungsvideo
LINK: RAD_SENSOR_VIDEO_URL
DESC:
Tutorial-Video zum Elios-3-Strahlungssensor—Montage, Flugverhalten, Datenhinweise, Aufnahme und Praxis vor Ort—vor den ersten RAD-Einsätzen ansehen.

### ID: useful_battery
LABEL: High-Capacity-Akku Start-Guide
LINK: HIGH_CAP_BATTERY_GUIDE_URL
DESC:
Video-Guide mit Vergleich von Standard- und High-Capacity-Akkus—Handling, Laden, Laufzeit und Unterschiede im Flugverhalten—vor dem Wechsel ansehen.

### ID: useful_tether
LABEL: Tether-Einheit Start-Guide
LINK: TETHER_UNIT_GUIDE_URL
DESC:
Video-Guide zur Tether-Einheit—Aufbau, sicherer Flugeinsatz, Wartung und Lagerung zwischen Einsätzen—vor dem ersten Tether-Flug ansehen.

### ID: useful_tent
LABEL: Flyability-Zelt Falt-Tutorial
LINK: TENT_FOLDING_TUTORIAL_URL
DESC:
Schritt-für-Schritt-Video, wie das Flyability-Zelt wieder in die Tasche gefaltet wird, ohne Stoff oder Stangen zu beschädigen—vor dem Zusammenpacken vor Ort ansehen.
