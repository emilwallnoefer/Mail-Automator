...
- **"Fly where people can't" (the Elios game)** — the drone game on the waiting-for-a-role screen (`components/role-gate.tsx`) and in the mail composer's preview slot, where it holds the space a draft will fill and `paused` stops the loop as the mail types in. Rules in `lib/elios-flight.ts`: pure, `Math.random()`-free and unit-tested, covering five spaces (boiler, ballast tank, mine stope, sewer, storage tank), 25 obstacle kinds built from seeded draws, and the bulkhead with a manhole that separates one space from the next. Everything visual is in `components/elios/` (`renderer.ts`, `obstacle-art.ts`, `lighting.ts`, `drone-art.ts`, `scenery.ts`, `paint.ts`, `look.ts`), mounted by `components/elios-game.tsx`. Leaderboard: `lib/elios-leaderboard.ts` + `app/api/elios-score/`, migration `supabase/2026-09-15-elios-leaderboard.sql`.
  ...
  - **The flight model and the spacing are fixed.** The leaderboard counts obstacles cleared, so touching `GRAVITY`, `FLAP_VELOCITY`, `OBSTACLE_GAP` or `MIN_PASSAGE` silently devalues every score already on the board. The single deliberate exception is `speedAt()`: the world accelerates from `SCROLL_SPEED` to `MAX_SCROLL_SPEED` over about a minute of flight, so a long run gets harder while the opening of every run stays exactly what it was. Obstacles are spaced in world units, so the ramp shortens the time to read one, never the distance between them — the renderer scrolls the scenery from the same function.
