...
- **Fleet (beta)** — material tracking + week-based booking, in `components/fleet/` (`fleet-panel.tsx` + `week-grid.tsx` + `reliability-badge.tsx`), mounted lazily from `dashboard-shell.tsx` and reachable from the burger menu. Pure rules in `lib/fleet-rules.ts` (week math, conflict checks, reliability score, reminder schedule — all `Date.now()`-free and covered by `fleet-rules.test.ts`); server reads in `lib/fleet-queries.ts`; all writes through `app/api/fleet/route.ts` on the service-role client, since the booking horizon and queue order cannot be expressed as RLS. Reminders: `app/api/cron/fleet-return-reminder/`. Migration: `supabase/2026-09-01-fleet-management.sql` (needs `btree_gist`; seeds the Elios 3 fleet). The reliability score is **derived, never stored** — keep it that way, or it will drift from the history it summarises.
