...
- `MAIL_BRIEF_MODEL` — optional **fallback** for the Brief-mode model. The model is normally chosen in the dashboard (Admin → Insights → Controls → "Mail brief model", stored in `workspace_settings.mail_brief_model`). This env var only applies when that setting is unset; if neither is set the default is `claude-opus-4-8`. Allowed values: `claude-opus-4-8` or `claude-sonnet-5`.
