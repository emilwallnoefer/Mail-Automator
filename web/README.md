...
- **Identity setup (once per person):** on first open, Fleet asks "is *X* you?" for the name that best matches. Yes claims it; No opens the full list of unclaimed names; "I'm not on the list" registers them under their own name via `register_member`. Gated on `identity_confirmed` — whether the user has any row in `fleet_holder_aliases` — so it is asked once per **person**, not once per browser, and survives clearing localStorage or switching device. Admins see the outstanding names as a quick list in the Manage tab.
