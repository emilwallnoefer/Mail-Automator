...
- **Schema (run in order):** `supabase/2026-09-01-fleet-management.sql`, then `-fleet-holder-claims.sql`, then `-fleet-assigned-pool.sql`. These are **structure only** — they create empty tables. The material list is entered and maintained through the app (Fleet → Manage, admin only), which is the only place it can be kept correct. An earlier set of data migrations imported a fleet and a year of bookings from the old spreadsheet; that import has been removed entirely.
