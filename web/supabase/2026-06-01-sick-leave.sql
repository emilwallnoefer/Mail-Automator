-- Add a "Sick leave" day type alongside the existing public-holiday flag.
--
-- Accounting rule (decided with the team):
--   A sick day is EXCUSED from the daily target (it never counts as a missing
--   weekday and never produces a deficit), but unlike a public holiday any
--   hours logged on a sick day do NOT count as overtime. So a sick day always
--   contributes exactly 0 to the overtime bank.
--
-- Safe to re-run.
...
-- ---------------------------------------------------------------------------
-- 1. Overtime bank: a sick day contributes 0 (no deficit, no overtime).
--    Mirrors the latest definition in 2026-04-17-skip-overtime-refresh-on-user-delete.sql
--    with one extra CASE branch.
-- ---------------------------------------------------------------------------
...
-- Recompute every existing user's bank so sick days that pre-date this column
-- (there are none yet, but keep the pattern) are reflected.
do $$
declare
  v_user uuid;
begin
...
-- ---------------------------------------------------------------------------
-- 2. Week read RPC: surface sick_leave per log.
-- ---------------------------------------------------------------------------
...
as $$
  ...
  week_logs as (
    select l.id, l.work_date, l.start_time, l.stop_time, l.net_mins, l.holiday, l.sick_leave
...
-- ---------------------------------------------------------------------------
-- 3. Admin overview: sick days are excused, so exclude them from missing days.
-- ---------------------------------------------------------------------------
