...
**T1.2 — Durable rate limiting.** **Code done 2026-07-31; needs provisioning.** `lib/security/rate-limit.ts` now uses an Upstash-compatible Redis REST store when configured and falls back to the in-memory `Map` otherwise (and on store error — a Redis blip weakens the limit, it does not remove it or fail the request). `checkRateLimit` is now **async**; all 8 call sites await it. Set either pair in Vercel to activate: `KV_REST_API_URL`+`KV_REST_API_TOKEN`, or `UPSTASH_REDIS_REST_URL`+`UPSTASH_REDIS_REST_TOKEN`. Until then behaviour is unchanged (per-instance counters). Effort remaining: provisioning only.
...
- 2026-07-31 — **Run-3 follow-ups shipped.** Everything left on the run-3 open list is now either done or reduced to a provisioning step.
  ...
  - **T1.2 code done** — durable rate-limit backend behind env config; `checkRateLimit` is now async. Provisioning a KV store is the remaining step.
  ...
  - **Still open, both provisioning rather than code:** a KV store for T1.2, and submitting the domain at hstspreload.org.
