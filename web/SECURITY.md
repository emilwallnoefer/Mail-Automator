...
- 2026-07-26 — **Audit run-3 completed** (3-agent recon fleet weighted to the areas runs 1–2 never examined: Postgres function privileges, the auth callback, the outbound-HTML builder after its T0.4 fix, cron CSRF, role-read consistency). Artifacts in `docs/security-audit/run-3/`; posture write-up at `docs/security-posture.md`.
  ...
  - **Still open:** T1.2 (durable rate limiting, needs an infra decision), T1.12 (`search_path` on nine RPCs), nonce-ing the theme bootstrap so `script-src` can drop `'unsafe-inline'`, HSTS preload submission, and the HR data-scope question (`guardTimeViewer` says "summaries" but `/api/admin/time-user` returns day-level sick leave and comp notes — deliberately left for a product decision).
