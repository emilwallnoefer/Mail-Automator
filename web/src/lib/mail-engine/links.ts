...
import type { Course, MailInlineAttachment, MailInput } from "./types";

export function resolveLinkUrl(linkKey: string, catalog: Course[]) {
...
export function resolveFeedbackQr(input: MailInput): { url: string; attachment?: MailInlineAttachment } {
  const links = trainingLinks as Record<string, string>;
...
export function resolveIncludedChangeIds(input: MailInput) {
...
export function buildTrainingMaterialsBlock(language: MailLanguage, selectedChangeIds: string[]) {
...
export function inferIndustryCourseIds(input: MailInput, catalog: Course[]) {
...
export function buildIndustryTrainingBlock(language: MailLanguage, selectedIds: string[], catalog: Course[]) {
...
export function buildUsefulLinksBlock(language: MailLanguage, selectedIds: string[], input: MailInput) {
...
export function buildUsefulLinksBlockFromChanges(language: MailLanguage, selectedChangeIds: string[]) {
  const selected = new Set(selectedChangeIds);
...
export function certificationNote(language: MailLanguage, enabled?: boolean, singular?: boolean) {
  if (!enabled) return "";
  if (language === "de") {
    return singular
...
export function simulatorNote(language: MailLanguage, enabled?: boolean) {
  if (!enabled) return "";
...
export function addonsBlock(language: MailLanguage, datasetsUrl?: string, singular?: boolean) {
